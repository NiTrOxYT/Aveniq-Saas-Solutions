...
export default function ArticlesIndexPage() {
    ...
    return (<div className="bg-black min-h-screen text-white selection:bg-[#6750A4]">
      <SEOHead title="Technical Engineering Articles & Architecture Guides | Aveniq" description="Deep technical articles on AI Agents, Model Context Protocol (MCP), Enterprise RAG, Multi-Tenant SaaS, Microservices, and Cloud Engineering." canonical="https://theaveniq.site/articles"/>
      <SchemaMarkup schema={[
            organizationSchema,
            {
      ...
      <main className="max-w-6xl mx-auto px-6 py-32">
        <ContentMetaHeader title="Technical Engineering Articles & Thought Leadership" subtitle="Deep-dive architecture guides, benchmark comparisons, production failure analyses, and code patterns written by Aveniq engineers." category="Engineering Publications" author="Aveniq Editorial Board" reviewer="VP of Engineering" lastUpdated="August 2026" version="v3.0.0"/>

        {/* Cluster Filter Buttons */}
        <div className="flex flex-wrap gap-2 mb-10 pb-4 border-b border-white/10">
          <button onClick={() => setSelectedCluster("All")} className={`px-4 py-2 rounded-full text-xs font-mono transition-all ${selectedCluster === "All"
            ? "bg-[#6750A4] text-white font-semibold"
            : "bg-white/5 text-white/60 hover:text-white border border-white/10"}`}>
            All Clusters ({editorialTopics.length})
          </button>
          {editorialClusters.map((cluster, idx) => (<button key={idx} onClick={() => setSelectedCluster(cluster)} className={`px-4 py-2 rounded-full text-xs font-mono transition-all ${selectedCluster === cluster
                ? "bg-[#6750A4] text-white font-semibold"
                : "bg-white/5 text-white/60 hover:text-white border border-white/10"}`}>
              {cluster}
            </button>))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16">
          {filteredTopics.map((topic) => (<Link key={topic.id} href={`/articles/${topic.slug}`} className="group p-6 rounded-2xl bg-zinc-950 border border-white/10 hover:border-[#9C89D9]/50 transition-all duration-300 flex flex-col justify-between">
              <div>
