...
export default function StartProjectPage() {
    ...
    const checkRateLimit = () => {
        try {
            const now = Date.now();
            const submissions = JSON.parse(localStorage.getItem("aveniq_submissions_timestamps") || "[]");
    ...
    return (<div className="relative min-h-screen bg-black text-white selection:bg-[#6750A4] selection:text-white overflow-x-hidden pt-12">
      <SEOHead title="Start Your Project | Aveniq — AI & Custom SaaS Development Partner" description="Launch your software project with Aveniq. Define your specifications, select your budget range, and partner with senior systems architects to build custom SaaS & AI solutions." canonical="https://theaveniq.site/start-project" keywords="Start project, custom software development, hire SaaS engineers, hire AI agent developers, software project estimate, software kickoff"/>
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10">
        {submitted ? (<div className="max-w-xl mx-auto text-center py-16 px-6 bg-[#08080a] border border-white/[0.06] rounded-3xl shadow-xl">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <button onClick={() => navigate("/")} className="w-full sm:w-auto px-6 py-3.5 bg-[#F5F5F5] hover:bg-white text-[#0A0A0A] rounded-xl text-sm font-semibold transition-colors cursor-pointer inline-flex items-center justify-center gap-2">
                Return Home
              </button>
            </div>
          </div>) : (<div className="flex flex-col lg:flex-row lg:items-start gap-12 lg:gap-16">
              ...
              <div className="bg-[#08080a] border border-white/[0.06] rounded-3xl p-6 sm:p-8 md:p-10 shadow-2xl relative overflow-hidden backdrop-blur-sm">
                <form className="space-y-8" onSubmit={handleSubmit}>
                  ...
                  <div className="hidden" style={{ position: "absolute", height: 0, width: 0, overflow: "hidden", opacity: 0, pointerEvents: "none" }}>
                    ...
                    <input type="text" id="website_url" name="website_url" value={websiteUrl} onChange={(e) => setWebsiteUrl(e.target.value)} tabIndex={-1} autoComplete="off"/>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="text" id="name" value={name} onChange={handleInputChange(setName)} className={`w-full h-12 bg-white/[0.03] border ${errors.name ? "border-rose-500/50 focus:ring-rose-500/30" : "border-white/10 focus:ring-[#9C89D9]/50"} rounded-xl px-4 py-3 focus:outline-none focus:ring-1 focus:border-transparent text-white placeholder:text-white/20 transition-all duration-200 text-sm`} placeholder="your name"/>
                        {errors.name && (<span className="block mt-1.5 text-xs text-rose-400 font-medium">{errors.name}</span>)}
                      </div>
                      <div>
                        ...
                        <input type="email" id="email" value={email} onChange={handleInputChange(setEmail)} className={`w-full h-12 bg-white/[0.03] border ${errors.email ? "border-rose-500/50 focus:ring-rose-500/30" : "border-white/10 focus:ring-[#9C89D9]/50"} rounded-xl px-4 py-3 focus:outline-none focus:ring-1 focus:border-transparent text-white placeholder:text-white/20 transition-all duration-200 text-sm`} placeholder="your.email@example.com"/>
                        {errors.email && (<span className="block mt-1.5 text-xs text-rose-400 font-medium">{errors.email}</span>)}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="text" id="company" value={company} onChange={handleInputChange(setCompany)} className={`w-full h-12 bg-white/[0.03] border ${errors.company ? "border-rose-500/50 focus:ring-rose-500/30" : "border-white/10 focus:ring-[#9C89D9]/50"} rounded-xl px-4 py-3 focus:outline-none focus:ring-1 focus:border-transparent text-white placeholder:text-white/20 transition-all duration-200 text-sm`} placeholder="your company"/>
                        {errors.company && (<span className="block mt-1.5 text-xs text-rose-400 font-medium">{errors.company}</span>)}
                      </div>
                      <div>
                        ...
                        <input type="tel" id="phone" value={phone} onChange={handleInputChange(setPhone)} className={`w-full h-12 bg-white/[0.03] border ${errors.phone ? "border-rose-500/50 focus:ring-rose-500/30" : "border-white/10 focus:ring-[#9C89D9]/50"} rounded-xl px-4 py-3 focus:outline-none focus:ring-1 focus:border-transparent text-white placeholder:text-white/20 transition-all duration-200 text-sm`} placeholder="+1 (555) 000-0000"/>
                        {errors.phone && (<span className="block mt-1.5 text-xs text-rose-400 font-medium">{errors.phone}</span>)}
                      </div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <div className="relative">
                          <select id="projectType" value={projectType} onChange={handleInputChange(setProjectType)} className={`w-full h-12 bg-white/[0.03] border ${errors.projectType ? "border-rose-500/50 focus:ring-rose-500/30" : "border-white/10 focus:ring-[#9C89D9]/50"} rounded-xl px-4 py-2.5 focus:outline-none focus:ring-1 focus:border-transparent text-white transition-all duration-200 appearance-none text-sm`}>
                        ...
                        {errors.projectType && (<span className="block mt-1.5 text-xs text-rose-400 font-medium">{errors.projectType}</span>)}
                      </div>
                      <div>
                        ...
                        <div className="relative">
                          <select id="contactMethod" value={contactMethod} onChange={handleInputChange(setContactMethod)} className={`w-full h-12 bg-white/[0.03] border ${errors.contactMethod ? "border-rose-500/50 focus:ring-rose-500/30" : "border-white/10 focus:ring-[#9C89D9]/50"} rounded-xl px-4 py-2.5 focus:outline-none focus:ring-1 focus:border-transparent text-white transition-all duration-200 appearance-none text-sm`}>
                        ...
                        {errors.contactMethod && (<span className="block mt-1.5 text-xs text-rose-400 font-medium">{errors.contactMethod}</span>)}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <div className="relative">
                          <select id="budgetRange" value={budgetRange} onChange={handleInputChange(setBudgetRange)} className={`w-full h-12 bg-white/[0.03] border ${errors.budgetRange ? "border-rose-500/50 focus:ring-rose-500/30" : "border-white/10 focus:ring-[#9C89D9]/50"} rounded-xl px-4 py-2.5 focus:outline-none focus:ring-1 focus:border-transparent text-white transition-all duration-200 appearance-none text-sm`}>
                        ...
                        {errors.budgetRange && (<span className="block mt-1.5 text-xs text-rose-400 font-medium">{errors.budgetRange}</span>)}
                      </div>
                      <div>
                        <label htmlFor="timeline" className="block text-xs font-semibold uppercase tracking-wider text-white/50 mb-2">Timeline</label>
                        <div className="relative">
                          <select id="timeline" value={timeline} onChange={handleInputChange(setTimeline)} className={`w-full h-12 bg-white/[0.03] border ${errors.timeline ? "border-rose-500/50 focus:ring-rose-500/30" : "border-white/10 focus:ring-[#9C89D9]/50"} rounded-xl px-4 py-2.5 focus:outline-none focus:ring-1 focus:border-transparent text-white transition-all duration-200 appearance-none text-sm`}>
                        ...
                        {errors.timeline && (<span className="block mt-1.5 text-xs text-rose-400 font-medium">{errors.timeline}</span>)}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      <div>
                        ...
                        <textarea id="message" value={message} onChange={handleInputChange(setMessage)} rows={4} className={`w-full bg-white/[0.03] border ${errors.message ? "border-rose-500/50 focus:ring-rose-500/30" : "border-white/10 focus:ring-[#9C89D9]/50"} rounded-xl px-4 py-3 focus:outline-none focus:ring-1 focus:border-transparent text-white placeholder:text-white/20 transition-all duration-200 text-sm`} placeholder="Provide details about what you want us to build, any specific technical requirements, or business goals..."></textarea>
                        {errors.message && (<span className="block mt-1.5 text-xs text-rose-400 font-medium">{errors.message}</span>)}
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Submit Button */}
                  <div className="pt-4">
                    <button type="submit" disabled={isSubmitting} className="w-full bg-[#F5F5F5] hover:bg-white text-[#0A0A0A] h-14 rounded-xl font-semibold text-sm transition-colors active:scale-[0.98] flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
                      {isSubmitting ? (<div className="w-5 h-5 rounded-full border border-black/20 border-t-black animate-spin"/>) : (<>Start Your Project <ArrowRight className="w-4 h-4"/></>)}
                    </button>
                  </div>
                </form>
              </div>
