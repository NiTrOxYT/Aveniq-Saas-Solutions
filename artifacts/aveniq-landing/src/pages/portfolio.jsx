...
function ProjectCardInner({ proj, idx }) {
    const getCardGlow = (i) => {
        const glows = [
    ...
    return (<>
      ...
      <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none z-0" style={{ background: getCardGlow(idx) }}/>
      ...
      <div className="h-64 relative bg-[#09090b] overflow-hidden shrink-0">
        {proj.imageUrl ? (<img src={proj.imageUrl} alt={proj.title} loading="lazy" className="w-full h-full object-cover opacity-80 group-hover:opacity-100 group-hover:scale-[1.025] transition-all duration-700"/>) : (
      ...
      <div className="p-8 flex flex-col justify-between grow relative z-10">
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            {proj.link && (<ArrowUpRight className="w-4 h-4 text-white/30 group-hover:text-white group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all duration-300"/>)}
          </div>
        ...
        {proj.link && (<div className="mt-8 pt-6 border-t border-white/[0.04] flex items-center gap-2 text-xs font-semibold text-white/30 group-hover:text-white transition-colors duration-300">
            Visit Project Site
...
export default function PortfolioPage() {
    ...
    return (<div className="relative bg-black min-h-screen text-white selection:bg-[#6750A4] selection:text-white overflow-x-hidden">
      <SEOHead title="Our Portfolio — Selected Engineering Works & SaaS Deliveries" description="Browse our selected engineering works, custom SaaS platforms, AI automation systems, and mobile applications custom-built by Aveniq." canonical="https://theaveniq.site/portfolio" keywords="Aveniq portfolio, software projects, SaaS case studies, AI automation works, mobile app designs"/>
      ...
      <main className="relative z-10 pt-32 md:pt-40 pb-32 max-w-7xl mx-auto px-6">
        ...
        <div className="max-w-3xl mb-20">
          <motion.p initial={reduce ? false : { opacity: 0, y: 12 }} animate={{ opacity: 0.4, y: 0 }} transition={{ duration: 0.6 }} className="text-[10px] font-mono tracking-[0.3em] uppercase text-white mb-6">
            Engineering Portfolio
          </motion.p>
          <motion.h1 initial={reduce ? false : { opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.23, 1, 0.32, 1] }} className="font-serif text-5xl sm:text-6xl md:text-7xl mb-6 text-white tracking-tight leading-[1.1]" style={{ fontFamily: "'Instrument Serif', serif" }}>
            Selected{" "}
            <span style={{
            fontStyle: "italic",
          ...
          <motion.p initial={reduce ? false : { opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.1, ease: [0.23, 1, 0.32, 1] }} className="text-white/45 text-base md:text-lg font-light leading-relaxed max-w-[55ch] font-sans" style={{ fontFamily: "Barlow, sans-serif" }}>
        ...
        {!loading && tags.length > 2 && (<div className="flex flex-wrap gap-2.5 mb-16 border-b border-white/[0.04] pb-8 relative z-20">
            {tags.map((tag) => (<button key={tag} onClick={() => setFilter(tag)} className={`relative px-5 py-2 rounded-full text-xs font-medium tracking-wide transition-all duration-300 cursor-pointer ${filter === tag
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16">
            {[1, 2, 3, 4].map((i) => (<div key={i} className="h-[420px] rounded-3xl border border-white/[0.06] bg-white/[0.01] animate-pulse overflow-hidden flex flex-col justify-between p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16 relative z-20">
            <AnimatePresence mode="wait">
              {filteredProjects.map((proj, i) => {
                ...
                if (reduce) {
                    return (<div key={proj.id} onClick={handleOpen} className={cardClass}>
                      <ProjectCardInner proj={proj} idx={i}/>
                    </div>);
                }
                return (<motion.div key={proj.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.6, delay: i * 0.05, ease: [0.23, 1, 0.32, 1] }} onClick={handleOpen} className={cardClass}>
                    <ProjectCardInner proj={proj} idx={i}/>
                  </motion.div>);
            })}
            </AnimatePresence>
