...
import { Wrench, CheckSquare, ArrowRight, DollarSign, Calendar } from "lucide-react";
import { Link } from "wouter";

export default function SaasCostEstimatorPage() {
  const [platformType, setPlatformType] = useState<string>("web");
  const [authType, setAuthType] = useState<string>("oauth");
  const [hasAI, setHasAI] = useState<boolean>(true);
  const [hasPayments, setHasPayments] = useState<boolean>(true);
  const [hasAdminDashboard, setHasAdminDashboard] = useState<boolean>(true);
  const [scalabilityLevel, setScalabilityLevel] = useState<string>("growth");
  ...
  return (
    <div className="bg-black min-h-screen text-white selection:bg-[#6750A4]">
      <SEOHead
        title="SaaS Development Cost & Timeline Estimator | Aveniq"
        description="Estimate software development budget, delivery timeline, and technical architecture specifications for your custom SaaS or enterprise platform."
        canonical="https://theaveniq.site/tools/saas-cost-estimator"
      />
      <SchemaMarkup
        schema={[
          organizationSchema,
      ...
      <main className="max-w-5xl mx-auto px-6 py-32">
        <ContentMetaHeader
          title="Interactive Custom SaaS Cost & Timeline Estimator"
          subtitle="Configure feature scope, authentication complexity, AI integration, and multi-tenancy requirements to generate an instant budget and timeline projection."
          category="Interactive Tool"
          author="SaaS Delivery Team"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-16">
          ...
          <div className="lg:col-span-7 p-6 sm:p-8 rounded-2xl bg-zinc-950 border border-white/10 space-y-6">
            ...
            <div className="space-y-3 pt-4 border-t border-white/10">
              ...
              <label className="flex items-center gap-3 text-sm text-white/80 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 accent-[#9C89D9] rounded"
                />
              ...
              <label className="flex items-center gap-3 text-sm text-white/80 cursor-pointer">
                <input
                  type="checkbox"
                  checked={hasAdminDashboard}
                  onChange={(e) => setHasAdminDashboard(e.target.checked)}
                  className="w-4 h-4 accent-[#9C89D9] rounded"
                />
            ...
            <div className="pt-4 border-t border-white/10">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={() => setScalabilityLevel("growth")}
                  className={`p-3 text-xs rounded-xl border font-medium text-left transition-all ${
                    scalabilityLevel === "growth"
                      ? "bg-[#6750A4]/20 border-[#9C89D9] text-white"
                      : "bg-white/5 border-white/10 text-white/60 hover:text-white"
                  }`}
                >
                ...
                <button
                  onClick={() => setScalabilityLevel("enterprise")}
                  className={`p-3 text-xs rounded-xl border font-medium text-left transition-all ${
