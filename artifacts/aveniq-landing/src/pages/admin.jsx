...
const ChartSkeleton = () => (<div className="h-64 w-full bg-[#08080a] border border-[#1a1a22] rounded-xl flex items-center justify-center animate-pulse">
...
export default function AdminPage() {
    ...
    const [projStatus, setProjStatus] = useState("Published");
    ...
    const [leadsSortField, setLeadsSortField] = useState("created_at");
    ...
    const fallbackEmailLogs = useMemo(() => [
    ...
    const fallbackActivityLogs = useMemo(() => [
    ...
    const handleResendMail = async (log) => {
        toast({
    ...
    const handleArchiveLead = async (lead) => {
        ...
        toast({
            ...
            action: (<ToastAction altText="Undo archiving" onClick={() => {
                    isReverted = true;
    ...
    const handleProjDragOver = (e, id) => {
        e.preventDefault();
        if (!draggedProjId || draggedProjId === id)
            return;
    ...
    const sourceStats = useMemo(() => {
        const statsMap = leads.reduce((acc, cur) => {
    ...
    const securityHealthCards = [
        { title: "Authentication Engine", status: "Healthy", desc: "Operator credentials security active", variant: "green" },
    ...
    const unifiedLeadTimeline = useMemo(() => {
        ...
        return [...notes, ...events].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
    ...
    const commandPaletteFilteredCommands = useMemo(() => {
        ...
        const matchedLeads = leads
            .filter(l => l.name.toLowerCase().includes(commandPaletteQuery.toLowerCase()) ||
        ...
        return [...navs, ...actions, ...matchedLeads].filter(item => item.label.toLowerCase().includes(commandPaletteQuery.toLowerCase()));
    ...
    const actionItemsCount = useMemo(() => {
        let count = 0;
        if (leads.some(l => l.status === "New"))
            count += leads.filter(l => l.status === "New").length;
        if (emailLogs.some(e => e.status === "Failed"))
            count += emailLogs.filter(e => e.status === "Failed").length;
        if (projects.some(p => p.status === "Draft"))
            count += projects.filter(p => p.status === "Draft").length;
        return count;
    }, [leads, emailLogs, projects]);
    if (authLoading) {
        return (<div className="min-h-screen bg-[#08080a] flex flex-col items-center justify-center select-none font-sans">
        <div className="w-5 h-5 rounded-full border border-white/5 border-t-[#10b981] animate-spin"/>
    ...
    if (!session || !isAdmin) {
        return (<div className="min-h-screen bg-[#08080a] text-[#f5f5f5] flex flex-col justify-center items-center px-4 relative select-none font-sans">
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }} className="w-full max-w-sm bg-[#0e0e11] border border-[#1a1a22] rounded-xl p-8 shadow-[0_24px_50px_-12px_rgba(0,0,0,0.9)]">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <input type="email" id="login-email" value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} className="w-full bg-[#08080a] border border-[#1a1a22] rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#10b981] text-white placeholder:text-white/20 text-xs font-light transition-colors" placeholder="operator@theaveniq.site" required/>
            </div>
            ...
            <div>
              ...
              <input type="password" id="login-password" value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} className="w-full bg-[#08080a] border border-[#1a1a22] rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#10b981] text-white placeholder:text-white/20 text-xs font-light transition-colors" placeholder="••••••••" required/>
            </div>

            <div className="pt-2">
              <button type="submit" disabled={loginLoading} className="w-full bg-[#f5f5f5] hover:bg-white text-[#08080a] py-3 rounded-lg font-bold text-xs transition-colors active:scale-[0.98] flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50">
                {loginLoading ? (<div className="w-4 h-4 rounded-full border border-[#08080a]/20 border-t-[#08080a] animate-spin"/>) : (<>Access Operations Console <ChevronRight className="w-4 h-4"/></>)}
              </button>
            </div>
          </form>
    ...
    return (<div className="min-h-screen bg-[#08080a] text-[#f5f5f5] flex font-sans antialiased select-none selection:bg-[#10b981]/30 relative overflow-x-hidden">
      
      {/* 1. Command Palette Modal (⌘K) */}
      <AnimatePresence>
        {isCommandPaletteOpen && (<div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh] px-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 0.5 }} exit={{ opacity: 0 }} onClick={() => setIsCommandPaletteOpen(false)} className="absolute inset-0 bg-black"/>
            <motion.div initial={{ opacity: 0, scale: 0.97, y: -10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97, y: -10 }} transition={{ duration: 0.15, ease: "easeOut" }} className="w-full max-w-lg bg-[#0e0e11] border border-[#1a1a22] rounded-xl overflow-hidden shadow-[0_32px_64px_rgba(0,0,0,0.9)] z-10 flex flex-col max-h-[400px]">
              <div className="relative border-b border-[#1a1a22] p-4 flex items-center gap-3">
                <Search className="w-4 h-4 text-[#a1a1aa]"/>
                <input type="text" autoFocus value={commandPaletteQuery} onChange={e => {
            ...
            }} className="bg-transparent border-none outline-none text-xs text-white placeholder:text-white/20 w-full font-light" placeholder="Type a command, tab name, or lead search query..."/>
                <span className="text-[9px] font-mono border border-[#1a1a22] bg-[#08080a] px-1.5 py-0.5 rounded text-[#a1a1aa]">ESC</span>
              </div>

              <div className="flex-1 overflow-y-auto p-2 space-y-1">
                {commandPaletteFilteredCommands.map((cmd, idx) => (<button key={idx} onClick={() => {
                    cmd.action();
                    setCommandPaletteQuery("");
                    setCommandPaletteIndex(0);
                }} onMouseEnter={() => setCommandPaletteIndex(idx)} className={`w-full flex items-center justify-between text-left px-3 py-2.5 text-xs rounded-lg transition-colors cursor-pointer ${commandPaletteIndex === idx ? "bg-white/[0.04] text-white" : "text-[#a1a1aa]"}`}>
                    <span>{cmd.label}</span>
                    {cmd.type === "command" && (<Terminal className="w-3.5 h-3.5 opacity-40"/>)}
                  </button>))}
                {commandPaletteFilteredCommands.length === 0 && (<p className="text-center py-6 text-xs text-[#a1a1aa] italic">No commands or operators matching queries.</p>)}
              </div>
            </motion.div>
          </div>)}
      </AnimatePresence>
      ...
      {confirmDialog?.isOpen && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-[#0e0e11] border border-[#1a1a22] rounded-xl p-6 w-full max-w-sm relative z-10 space-y-4 shadow-2xl">
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button onClick={() => setConfirmDialog(null)} className="px-3.5 py-2 bg-transparent hover:bg-white/5 border border-[#1a1a22] text-[#a1a1aa] hover:text-white rounded-lg text-xs font-bold transition-colors cursor-pointer">
                Cancel
              </button>
              <button onClick={() => {
      ...
      {editingProject && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <aside className={`hidden lg:flex flex-col bg-[#0e0e11] border-r border-[#1a1a22] p-5 shrink-0 relative z-30 justify-between transition-all duration-300 ${isSidebarCollapsed ? "w-[68px]" : "w-[260px]"}`}>
        <div>
          <div className="flex items-center justify-between mb-8 px-1">
            {!isSidebarCollapsed && (<div className="flex items-center gap-3">
                <span className="w-2.5 h-2.5 rounded-full bg-[#10b981] animate-pulse"/>
                <span className="text-sm font-semibold tracking-wide text-white font-mono">Aveniq Operating OS</span>
              </div>)}
            <button onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} className={`p-1.5 rounded border border-[#1a1a22] bg-[#08080a]/60 hover:bg-[#08080a] text-[#a1a1aa] hover:text-white transition-colors cursor-pointer ${isSidebarCollapsed ? "mx-auto" : ""}`}>
          ...
          <nav className="space-y-1">
            ...
            return (<button key={item.id} onClick={() => {
                    setActiveTab(item.id);
                    setSelectedLead(null);
                }} className={`w-full flex items-center px-3 py-2 text-xs font-semibold rounded-lg transition-colors cursor-pointer justify-start group border ${isActive
                    ? "bg-white/[0.04] text-white border-[#1a1a22]"
                    : "text-[#a1a1aa] hover:text-white hover:bg-white/[0.01] border-transparent"}`} title={isSidebarCollapsed ? item.label : undefined}>
                  <Icon className={`w-4 h-4 shrink-0 ${isActive ? "text-[#10b981]" : "text-[#a1a1aa]"} ${isSidebarCollapsed ? "mx-auto" : "mr-3"}`}/>
                  {!isSidebarCollapsed && (<div className="flex-1 flex items-center justify-between">
        ...
        <div className="pt-6 border-t border-[#1a1a22] space-y-4">
          ...
          <div className={`flex items-center gap-3 px-1 ${isSidebarCollapsed ? "justify-center" : ""}`}>
            ...
            {!isSidebarCollapsed && (<div className="truncate">
          ...
          {!isSidebarCollapsed && (<div className="bg-[#08080a] border border-[#1a1a22] rounded-lg p-3 space-y-2.5 text-[9px] font-mono text-[#a1a1aa] select-none">
      ...
      <div className="lg:hidden fixed top-0 inset-x-0 h-14 bg-[#0e0e11] border-b border-[#1a1a22] flex items-center justify-between px-4 z-40">
        ...
        <button onClick={() => setIsMobileSidebarOpen(!isMobileSidebarOpen)} className="p-1.5 text-[#a1a1aa] hover:text-white cursor-pointer">
      ...
      <AnimatePresence>
        {isMobileSidebarOpen && (<>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 0.5 }} exit={{ opacity: 0 }} onClick={() => setIsMobileSidebarOpen(false)} className="lg:hidden fixed inset-0 bg-black z-40"/>
            <motion.aside initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="lg:hidden fixed inset-y-0 left-0 w-[260px] bg-[#0e0e11] border-r border-[#1a1a22] p-5 z-50 flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between mb-8 px-1">
                  <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => setIsMobileSidebarOpen(false)} className="p-1 text-[#a1a1aa] hover:text-white cursor-pointer">
                    <X className="w-4 h-4"/>
                  </button>
                </div>
                ...
                return (<button key={item.id} onClick={() => {
                    ...
                    }} className={`w-full flex items-center gap-3 px-3 py-2.5 text-xs font-semibold rounded-lg transition-colors cursor-pointer justify-start ${isActive
      ...
      <main className="flex-1 min-w-0 flex flex-col pt-14 lg:pt-0 overflow-x-hidden relative z-10 bg-[#08080a]">
        <header className="h-14 border-b border-[#1a1a22] bg-[#08080a]/60 sticky top-0 px-6 flex items-center justify-between z-20">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setIsCommandPaletteOpen(true)} className="hidden sm:flex items-center gap-2 text-xs text-[#a1a1aa] border border-[#1a1a22] rounded-lg px-3 py-1 bg-white/[0.01] hover:bg-white/[0.03] transition-colors cursor-pointer">
              <Search className="w-3.5 h-3.5"/> Search Command Palette <span className="text-[9px] font-mono opacity-50">⌘K</span>
            </button>
            <button onClick={fetchDashboardData} className="flex items-center justify-center p-1.5 border border-[#1a1a22] rounded-lg bg-white/[0.01] hover:bg-white/[0.03] text-[#a1a1aa] hover:text-white cursor-pointer active:scale-95 transition-all">
        ...
        <div className="p-6 md:p-8 max-w-7xl w-full mx-auto flex-1 flex flex-col gap-6">
          {dataLoading ? (<div className="flex-1 flex flex-col gap-6">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                {[1, 2, 3, 4].map(n => (<div key={n} className="h-24 bg-[#0e0e11] border border-[#1a1a22] rounded-xl animate-pulse"/>))}
              </div>
              ...
              {activeTab === "overview" && (<div className="space-y-6">
                  ...
                  {actionItemsCount > 0 && (<div className="bg-amber-500/5 border border-amber-500/20 rounded-xl p-4 flex items-center justify-between text-xs text-amber-200">
                      <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => {
                        if (leads.some(l => l.status === "New"))
                            setActiveTab("leads");
                        else if (emailLogs.some(e => e.status === "Failed"))
                            setActiveTab("emails");
                        else
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-[#0e0e11] border border-[#1a1a22] rounded-xl p-5 flex flex-col justify-between gap-5 lg:col-span-2">
                      <div>
                        ...
                        <div className="space-y-3.5 max-h-[220px] overflow-y-auto pr-1">
                          {activityLogs.slice(0, 8).map(log => (<div key={log.id} className="flex justify-between items-start gap-4 text-xs font-light">
                              <div className="flex gap-2">
                                ...
                                {log.details?.email && (<span className="text-[#a1a1aa]">({log.details.email})</span>)}
                              </div>
                      ...
                      <div className="flex gap-3 border-t border-[#1a1a22] pt-4">
                        <button onClick={() => setActiveTab("leads")} className="px-4 py-2 bg-[#f5f5f5] hover:bg-white text-[#08080a] text-xs font-bold rounded-lg cursor-pointer transition-colors">
                          CRM Center
                        </button>
                        <button onClick={handleExportLeads} className="px-4 py-2 bg-transparent border border-[#1a1a22] hover:bg-white/[0.02] text-white text-xs font-bold rounded-lg cursor-pointer transition-all active:scale-[0.98]">
                          Export Leads CSV
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="bg-[#0e0e11] border border-[#1a1a22] rounded-xl p-5 space-y-4">
                      ...
                      <div className="space-y-3.5">
                        {funnelSteps.map((step, idx) => (<div key={idx} className="space-y-1 text-xs">
                            ...
                            <div className="h-1 w-full bg-[#08080a] border border-[#1a1a22] rounded overflow-hidden">
                              <div className="h-full bg-[#10b981]" style={{ width: `${step.percentage}%` }}/>
                            </div>
                          </div>))}
                      </div>
                    </div>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-left border-collapse text-xs">
                        ...
                        <tbody className="divide-y divide-[#1a1a22]">
                          {leads.slice(0, 4).map(lead => (<tr key={lead.id} onClick={() => {
                        setSelectedLead(lead);
                        fetchLeadNotes(lead.id);
                        setActiveTab("leads");
              ...
              {activeTab === "leads" && (<div className="space-y-6">
                  <div className="flex flex-col xl:flex-row gap-6 items-start">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                          ...
                          <div className="relative">
                            ...
                            <input type="text" value={leadsSearch} onChange={e => setLeadsSearch(e.target.value)} className="bg-[#08080a] border border-[#1a1a22] text-xs text-white placeholder:text-white/20 rounded-lg pl-9 pr-4 py-2 w-48 focus:outline-none focus:border-[#10b981] font-light" placeholder="Search leads name/email..."/>
                          </div>
                          {/* Filter by Status */}
                          <div className="relative">
                            <select value={leadsFilterStatus} onChange={e => setLeadsFilterStatus(e.target.value)} className="bg-[#08080a] border border-[#1a1a22] text-xs text-white rounded-lg px-3 py-2 focus:outline-none focus:border-[#10b981] cursor-pointer pr-8 appearance-none">
                    ...
                    setLeadsSortAsc(!leadsSortAsc);
                          ...
                          <tbody className="divide-y divide-[#1a1a22]">
                            {paginatedLeads.map(lead => (<tr key={lead.id} onClick={() => {
                        setSelectedLead(lead);
                        fetchLeadNotes(lead.id);
                    }} className={`hover:bg-white/[0.01] cursor-pointer transition-colors ${selectedLead?.id === lead.id ? "bg-white/[0.02]" : ""} group`}>
                                <td className="py-3 pr-4 font-semibold text-white">
                                  <div className="flex items-center gap-3">
                                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-mono shrink-0 ${getAvatarColor(lead.name)}`}>
                                      {getInitials(lead.name)}
                        ...
                        fetchLeadNotes(lead.id);
                                      ...
                                      {lead.status !== "Contacted" && (<button onClick={() => handleStatusChange(lead.id, "Contacted")} className="px-2 py-1 bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 rounded text-[10px] font-semibold border border-amber-500/20 cursor-pointer">
                                          Contacted
                                        </button>)}
                                      <button onClick={() => handleArchiveLead(lead)} className="px-2 py-1 bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 rounded text-[10px] font-semibold border border-rose-500/20 cursor-pointer">
                                        Archive
                                      </button>
                                    </div>
                                    <button onClick={() => {
                      ...
                      <div className="flex items-center justify-between border-t border-[#1a1a22] pt-4 text-xs">
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={() => setLeadsPage(Math.max(1, leadsPage - 1))} disabled={leadsPage === 1} className="p-1 border border-[#1a1a22] rounded bg-[#08080a] hover:bg-white/[0.02] text-[#a1a1aa] disabled:opacity-30 cursor-pointer transition-colors">
                            <ChevronLeft className="w-4 h-4"/>
                          </button>
                          <span className="text-white font-mono">{leadsPage} / {totalPages}</span>
                          <button onClick={() => setLeadsPage(Math.min(totalPages, leadsPage + 1))} disabled={leadsPage === totalPages} className="p-1 border border-[#1a1a22] rounded bg-[#08080a] hover:bg-white/[0.02] text-[#a1a1aa] disabled:opacity-30 cursor-pointer transition-colors">
                    ...
                    <AnimatePresence>
                      {selectedLead && (<motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="w-full xl:w-[480px] bg-[#0e0e11] border border-[#1a1a22] rounded-xl overflow-hidden flex flex-col justify-between shrink-0 relative shadow-2xl h-[560px]">
                          {/* Drawer Header */}
                          <div className="flex items-center justify-between p-4 border-b border-[#1a1a22] bg-[#08080a]/30">
                            ...
                            <button onClick={() => setSelectedLead(null)} className="p-1 text-[#a1a1aa] hover:text-white cursor-pointer">
                              <X className="w-4 h-4"/>
                            </button>
                          </div>
                            ...
                            <div className="w-1/2 p-4 space-y-5 overflow-y-auto flex flex-col justify-between h-full">
                              <div className="space-y-4 overflow-y-auto flex-1 min-h-0">
                                ...
                                <div className="space-y-4 pr-1">
                                  {unifiedLeadTimeline.map(item => (<div key={item.id} className="relative pl-4 border-l border-white/[0.04] space-y-1">
                                      <div className="absolute -left-[3px] top-1.5 w-1.5 h-1.5 rounded-full bg-[#1a1a22] border border-[#10b981]/40"/>
                                      {item.type === "note" ? (<div className="bg-[#08080a]/60 border border-[#1a1a22] rounded-lg p-2 space-y-1">
                                        ...
                                        </div>) : (<div className="text-[10px] text-[#a1a1aa] font-light">
                                  ...
                                  {unifiedLeadTimeline.length === 0 && (<p className="text-[10px] text-[#a1a1aa] italic py-4 text-center">Timeline log empty.</p>)}
                                </div>
                              </div>

                              <form onSubmit={handleAddNote} className="space-y-2 border-t border-[#1a1a22] pt-3 shrink-0">
                                <textarea value={newNote} onChange={e => setNewNote(e.target.value)} rows={2} required placeholder="Post comments to timeline..." className="w-full bg-[#08080a] border border-[#1a1a22] rounded-lg p-2 focus:outline-none focus:border-[#10b981] text-white placeholder:text-white/20 text-xs font-light resize-none"/>
                                <button type="submit" disabled={submittingNote} className="px-3 py-1 bg-[#f5f5f5] hover:bg-white text-[#08080a] text-[10px] font-bold rounded cursor-pointer transition-colors">
                          ...
                          <div className="p-3 border-t border-[#1a1a22] bg-[#0e0e11] flex flex-wrap gap-1.5 shrink-0 justify-between items-center">
                            ...
                            <div className="flex gap-1.5">
                              {["Contacted", "Proposal Sent", "Won", "Lost"].map(st => (<button key={st} onClick={() => handleStatusChange(selectedLead.id, st)} className={`px-2.5 py-1 rounded text-[10px] font-semibold transition-all cursor-pointer border ${selectedLead.status === st
              ...
              {activeTab === "submissions" && (<div className="bg-[#0e0e11] border border-[#1a1a22] rounded-xl p-5 space-y-5">
                  <div className="flex items-center justify-between border-b border-[#1a1a22] pb-4">
                    ...
                    <button onClick={handleExportLeads} className="flex items-center gap-1.5 text-xs text-[#a1a1aa] hover:text-white border border-[#1a1a22] rounded-lg px-3 py-1.5 bg-[#08080a] hover:bg-[#08080a]/60 transition-colors cursor-pointer">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse text-xs">
                      ...
                      <tbody className="divide-y divide-[#1a1a22]">
                        {leads.map(lead => (<tr key={lead.id} className="hover:bg-white/[0.01]">
                        ...
                        setActiveTab("leads");
                              ...
                              <button onClick={() => handleArchiveLead(lead)} className="p-1.5 text-[#a1a1aa] hover:text-rose-400 cursor-pointer inline-block">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                      ...
                      <form onSubmit={handleAddProject} className="space-y-4 text-xs">
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <input type="text" id="title" value={title} onChange={e => setTitle(e.target.value)} className="w-full bg-[#08080a] border border-[#1a1a22] rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#10b981] text-white font-light" placeholder="e.g. Nexus Dashboard" required/>
                          </div>
                          ...
                          <div>
                            ...
                            <input type="text" id="tag" value={tag} onChange={e => setTag(e.target.value)} className="w-full bg-[#08080a] border border-[#1a1a22] rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#10b981] text-white font-light" placeholder="e.g. SaaS · CRM" required/>
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <textarea id="desc" value={desc} onChange={e => setDesc(e.target.value)} rows={3} className="w-full bg-[#08080a] border border-[#1a1a22] rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#10b981] text-white font-light" placeholder="Explain the project scope..." required/>
                        </div>

                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <label className="block text-[9px] font-semibold uppercase tracking-wider text-[#a1a1aa] mb-2 font-mono">Status</label>
                            <select value={projStatus} onChange={e => setProjStatus(e.target.value)} className="w-full bg-[#08080a] border border-[#1a1a22] rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#10b981] text-white cursor-pointer">
                              <option value="Published">Published</option>
                              <option value="Draft">Draft</option>
                              <option value="In Review">In Review</option>
                            </select>
                          ...
                          <div>
                            ...
                            <input type="url" id="link" value={link} onChange={e => setLink(e.target.value)} className="w-full bg-[#08080a] border border-[#1a1a22] rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#10b981] text-white font-light" placeholder="https://example.com"/>
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div onDragEnter={handleDrag} onDragOver={handleDrag} onDragLeave={handleDrag} onDrop={handleDrop} className={`w-full rounded-lg border-2 border-dashed ${dragActive ? "border-[#10b981] bg-[#10b981]/5" : "border-[#1a1a22] hover:border-[#10b981]/30 bg-[#08080a]"} transition-colors text-center relative overflow-hidden flex flex-col justify-center items-center py-6 px-4 cursor-pointer`}>
                            <input type="file" id="file-upload" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-20" onChange={handleFileInput} accept=".jpg,.jpeg,.png,.webp"/>

                            {isUploading ? (<div className="flex flex-col items-center py-2">
                              ...
                              </div>) : imageUrl ? (<div className="relative group w-full max-h-[120px] rounded-lg overflow-hidden flex items-center justify-center bg-black/60 border border-[#1a1a22] p-1 z-30">
                        ...
                        setImageUrl("");
                          ...
                          {uploadError && (<span className="block mt-1.5 text-xs text-rose-400">{uploadError}</span>)}
                        </div>

                        <div className="pt-2">
                          <button type="submit" className="w-full bg-[#f5f5f5] hover:bg-white text-[#08080a] py-3 rounded-lg font-bold text-xs transition-colors cursor-pointer flex items-center justify-center gap-1.5">
                    ...
                    <div className="sticky top-20 space-y-4">
                      ...
                      <div className="relative h-[340px] rounded-xl border border-[#1a1a22] overflow-hidden bg-[#0e0e11]">
                        {imageUrl.trim() && (<img src={imageUrl.trim()} alt="Mockup" className="absolute inset-0 w-full h-full object-cover opacity-20 z-0"/>)}
                  ...
                  <div className="bg-[#0e0e11] border border-[#1a1a22] rounded-xl p-5 space-y-6">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        <div className="relative">
                          <Search className="w-3.5 h-3.5 absolute left-3 top-1/2 -translate-y-1/2 text-[#a1a1aa]"/>
                          <input type="text" value={portfolioSearch} onChange={e => setPortfolioSearch(e.target.value)} className="bg-[#08080a] border border-[#1a1a22] text-xs text-white placeholder:text-white/20 rounded-lg pl-9 pr-4 py-2 w-48 focus:outline-none focus:border-[#10b981] font-light" placeholder="Search portfolio..."/>
                        </div>
                        <div className="relative">
                          <select value={portfolioFilter} onChange={e => setPortfolioFilter(e.target.value)} className="bg-[#08080a] border border-[#1a1a22] text-xs text-white rounded-lg px-3 py-2 focus:outline-none focus:border-[#10b981] cursor-pointer pr-8 appearance-none">
                            <option value="all">All Tags</option>
                            {Array.from(new Set(projects.map(p => p.tag))).map(tg => (<option key={tg} value={tg}>{tg}</option>))}
                          </select>
                          <Filter className="w-3.5 h-3.5 text-[#a1a1aa] absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none"/>
                        </div>
                        <div className="relative">
                          <select value={portfolioStatusFilter} onChange={e => setPortfolioStatusFilter(e.target.value)} className="bg-[#08080a] border border-[#1a1a22] text-xs text-white rounded-lg px-3 py-2 focus:outline-none focus:border-[#10b981] cursor-pointer pr-8 appearance-none">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {filteredProjects.map((proj, idx) => (<div key={proj.id} draggable onDragStart={(e) => handleProjDragStart(e, proj.id)} onDragOver={(e) => handleProjDragOver(e, proj.id)} onDragEnd={handleProjDragEnd} className={`group relative bg-[#08080a] border rounded-xl p-5 flex flex-col justify-between hover:border-white/10 transition-colors duration-200 cursor-grab active:cursor-grabbing ${draggedProjId === proj.id ? "opacity-30 border-dashed border-[#10b981]" : "border-[#1a1a22]"}`}>
                          <div className="space-y-4">
                            ...
                            <div className="h-32 rounded-lg bg-[#0e0e11] border border-[#1a1a22] overflow-hidden flex items-center justify-center relative">
                              {proj.imageUrl ? (<img src={proj.imageUrl} alt="" className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-300"/>) : (<Briefcase className="w-6 h-6 text-[#a1a1aa]"/>)}
                              <span className="absolute top-2 left-2 text-[9px] font-mono border border-[#1a1a22] bg-[#0e0e11] px-2 py-0.5 rounded text-[#10b981]">
                                {proj.tag}
                              </span>
                              <span className={`absolute top-2 right-2 text-[9px] font-mono border px-2 py-0.5 rounded ${proj.status === "Published"
                        ...
                        : proj.status === "Draft"
                          ...
                          <div className="border-t border-[#1a1a22] pt-4 mt-5 flex items-center justify-between text-xs">
                            ...
                            <div className="flex items-center gap-3">
                              <button onClick={() => setEditingProject(proj)} className="p-1 hover:bg-white/5 border border-[#1a1a22] rounded text-[#a1a1aa] hover:text-white cursor-pointer transition-colors" title="Edit layout configuration">
                                <Edit2 className="w-3.5 h-3.5"/>
                              </button>
                              <button onClick={() => handleDuplicateProject(proj)} className="p-1 hover:bg-white/5 border border-[#1a1a22] rounded text-[#a1a1aa] hover:text-white cursor-pointer transition-colors" title="Duplicate layout project">
                                <Copy className="w-3.5 h-3.5"/>
                              </button>
                              {proj.link && (<a href={proj.link} target="_blank" rel="noreferrer" className="p-1 hover:bg-white/5 border border-[#1a1a22] rounded text-[#a1a1aa] hover:text-white" title="Open live link">
                                  <Globe className="w-3.5 h-3.5"/>
                                </a>)}
                              <button onClick={() => handleDeleteProject(proj.id, proj.title)} className="p-1 hover:bg-rose-500/10 border border-transparent rounded text-[#a1a1aa] hover:text-rose-400 cursor-pointer transition-colors">
                                <Trash2 className="w-3.5 h-3.5"/>
                              </button>
                            </div>
                          </div>
                  ...
                  <div className="bg-[#0e0e11] border border-[#1a1a22] rounded-xl p-5 space-y-6">
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-left border-collapse text-xs">
                        ...
                        <tbody className="divide-y divide-[#1a1a22]">
                          {filteredEmailLogs.map(log => (<tr key={log.id} className="hover:bg-white/[0.01]">
                              ...
                              <td className="py-3 px-4 align-middle">
                                <div className="space-y-1">
                                  <span className={`px-2 py-0.5 rounded text-[9px] font-semibold font-mono border ${log.status === "Delivered" || log.status === "Sent"
                        ? "bg-emerald-500/5 text-emerald-400 border-emerald-500/20"
                        : log.status === "Queued"
                            ...
                            : "bg-rose-500/5 text-rose-400 border-rose-500/20"}`}>
                                  ...
                                  {log.error_message && (<p className="text-[9px] text-rose-400/80 font-mono max-w-[200px] truncate" title={log.error_message}>
                                      {log.error_message}
                                    </p>)}
                                </div>
                              </td>
                              ...
                              <td className="py-3 pl-4 text-right align-middle">
                                {log.status === "Failed" && (<button onClick={() => handleResendMail(log)} className="px-2.5 py-1 text-[10px] border border-[#1a1a22] bg-[#08080a] text-white hover:text-[#10b981] font-bold rounded cursor-pointer transition-colors">
                                    Retry
                                  </button>)}
                              </td>
                            </tr>))}
              ...
              {activeTab === "analytics" && (<div className="space-y-6">
                  ...
                  <div className="bg-[#0e0e11] border border-[#1a1a22] rounded-xl p-6 space-y-6">
                    ...
                    <div className="space-y-4">
                      {funnelSteps.map((step, idx) => (<div key={idx} className="space-y-2 text-xs">
                          <div className="flex justify-between items-center font-light">
                            <span className="text-[#a1a1aa]">{step.label}</span>
                            <span className="font-mono text-[10px] text-white">{step.count} ({step.percentage}%)</span>
                          </div>
                          <div className="h-2 w-full bg-[#08080a] border border-[#1a1a22] rounded overflow-hidden">
                            <div className="h-full bg-[#10b981] transition-all duration-500" style={{ width: `${step.percentage}%` }}/>
                          </div>
                        </div>))}
                    </div>
                  </div>
              ...
              {activeTab === "security" && (<div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {securityHealthCards.map((card, idx) => (<div key={idx} className="bg-[#0e0e11] border border-[#1a1a22] rounded-xl p-5 flex flex-col justify-between min-h-[120px]">
                        <div className="space-y-1">
                        ...
                        <div className="flex items-center justify-between border-t border-[#1a1a22] pt-3.5 mt-4 text-[10px]">
                          ...
                          <span className={`px-2 py-0.5 rounded text-[9px] font-bold font-mono ${card.variant === "green"
                        ? "bg-emerald-500/5 text-emerald-400 border border-emerald-500/20"
                        : "bg-amber-500/5 text-amber-400 border border-amber-500/20"} flex items-center gap-1`}>
                  ...
                  <div className="bg-[#0e0e11] border border-[#1a1a22] rounded-xl p-5">
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-left border-collapse text-xs">
                        ...
                        <tbody className="divide-y divide-[#1a1a22]">
                          {adminUsers.map(user => (<tr key={user.id} className="hover:bg-white/[0.01]">
                  ...
                  <form onSubmit={handleSaveSettings} className="space-y-6 text-xs max-w-2xl">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <input type="text" id="company-name" value={settings.company_name} onChange={e => setSettings({ ...settings, company_name: e.target.value })} className="w-full bg-[#08080a] border border-[#1a1a22] rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#10b981] text-white font-light" required/>
                      </div>
                      ...
                      <div>
                        ...
                        <input type="email" id="support-email" value={settings.support_email} onChange={e => setSettings({ ...settings, support_email: e.target.value })} className="w-full bg-[#08080a] border border-[#1a1a22] rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#10b981] text-white font-light" required/>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <input type="url" id="calendly-url" value={settings.calendly_url} onChange={e => setSettings({ ...settings, calendly_url: e.target.value })} className="w-full bg-[#08080a] border border-[#1a1a22] rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#10b981] text-white font-light" required/>
                    </div>
                    ...
                    <div className="space-y-3.5 pt-2">
                      ...
                      <div className="flex items-center justify-between bg-[#08080a] p-4 border border-[#1a1a22] rounded-lg">
                        ...
                        <select value={settings.default_lead_status} onChange={e => setSettings({ ...settings, default_lead_status: e.target.value })} className="bg-[#0e0e11] border border-[#1a1a22] text-xs text-white rounded-lg px-3 py-1.5 focus:outline-none focus:border-[#10b981] cursor-pointer">
                          <option value="New">New</option>
                          <option value="Contacted">Contacted</option>
                          <option value="Qualified">Qualified</option>
                        </select>
                    ...
                    <div className="flex items-center justify-between bg-[#08080a] p-4 border border-[#1a1a22] rounded-lg">
                      ...
                      <input type="checkbox" checked={settings.notification_settings.email} onChange={e => setSettings({
                    ...settings,
                    ...
                    <div className="pt-4 flex gap-4">
                      <button type="submit" className="px-5 py-3 bg-white text-[#08080a] font-bold rounded-lg cursor-pointer transition-colors flex items-center justify-center gap-1.5">
                        <Check className="w-4 h-4"/> Save Configuration Settings
                      </button>
                      <button type="button" onClick={handleLogout} className="px-5 py-3 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 font-bold border border-rose-500/20 rounded-lg cursor-pointer transition-colors">
                        Sign Out Operator
                      </button>
                    </div>
                  </form>
