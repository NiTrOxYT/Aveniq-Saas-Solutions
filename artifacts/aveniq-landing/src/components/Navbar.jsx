...
export default function Navbar() {
    ...
    useEffect(() => {
        gsap.fromTo(navRef.current, { y: "-100%", opacity: 0 }, { y: "0%", opacity: 1, duration: 1, delay: 1, ease: "power3.out" });
        const handleScroll = () => {
    ...
    return (<nav ref={navRef} className="fixed top-0 left-0 right-0 z-50 py-4 md:py-6 transition-all duration-300 border-transparent bg-transparent">
      <div className="container mx-auto px-4 md:px-6 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm font-medium text-white/70">
          ...
          <div className="relative" onMouseEnter={() => setServicesOpen(true)} onMouseLeave={() => setServicesOpen(false)}>
            <button className="hover:text-white transition-colors flex items-center gap-1 cursor-pointer" aria-expanded={servicesOpen} aria-haspopup="true">
              Services
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className={`transition-transform duration-200 ${servicesOpen ? "rotate-180" : ""}`}>
                <path d="M6 9l6 6 6-6"/>
              </svg>
            </button>

            {servicesOpen && (<div className="absolute top-full left-1/2 -translate-x-1/2 pt-3" role="menu">
                <div className="w-56 rounded-xl border border-white/[0.08] py-2 overflow-hidden" style={{
                background: "rgba(9,9,11,0.97)",
                backdropFilter: "blur(20px)",
                boxShadow: "0 20px 60px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.06)",
            }}>
                  {SERVICE_LINKS.map((link) => (<Link key={link.href} href={link.href} role="menuitem" onClick={() => setServicesOpen(false)} onMouseEnter={() => prefetchRoute(link.href)} onFocus={() => prefetchRoute(link.href)} className="block px-4 py-2.5 text-xs font-medium text-white/55 hover:text-white hover:bg-white/[0.04] transition-all duration-150">
                      {link.label}
                    </Link>))}
                </div>
              </div>)}
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <button onClick={handleStartProjectClick} onMouseEnter={() => prefetchRoute("/start-project")} onFocus={() => prefetchRoute("/start-project")} className="bg-gradient-to-r from-[#6750A4] to-[#9C89D9] text-white px-5 py-2 md:px-6 md:py-2.5 rounded-full font-semibold text-sm transition-all duration-200 active:scale-[0.97] hover:brightness-110 cursor-pointer">
            Start Your Project
          </button>
        </div>

        {/* Mobile toggle */}
        <button className="md:hidden text-white/70 hover:text-white transition-colors" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Toggle menu">
          {mobileOpen ? <X className="w-6 h-6"/> : <Menu className="w-6 h-6"/>}
        </button>
      </div>

      {/* Mobile menu */}
      {mobileOpen && (<div className="md:hidden absolute top-full left-0 right-0 bg-black/95 backdrop-blur-xl border-b border-white/10 py-6 px-4 max-h-[85vh] overflow-y-auto">
          <div className="flex flex-col gap-1 text-sm font-medium text-white/70">
            ...
            <div className="py-3 px-2">
              ...
              <div className="grid grid-cols-2 gap-1">
                {SERVICE_LINKS.map((link) => (<Link key={link.href} href={link.href} onClick={() => setMobileOpen(false)} className="text-xs text-white/55 hover:text-white py-2 px-3 rounded-lg hover:bg-white/[0.04] transition-all duration-150">
                    {link.label}
                  </Link>))}
              </div>
            </div>
            ...
            {["Portfolio", "About", "Contact"].map((label) => {
                ...
                return (<a key={label} href={targetHref} onClick={(e) => {
                        if (isContact) {
                            e.preventDefault();
                            navigate("/contact");
                        }
            ...
            <button onClick={handleStartProjectClick} className="bg-gradient-to-r from-[#6750A4] to-[#9C89D9] text-white px-6 py-3 rounded-full font-semibold text-sm transition-all duration-200 active:scale-[0.97] hover:brightness-110 mt-3 cursor-pointer">
              Start Your Project
            </button>
          </div>
        </div>)}
