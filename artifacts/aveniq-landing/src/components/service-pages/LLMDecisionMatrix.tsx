...
interface LLMDecisionMatrixProps {
  llmo: LLMOData;
}

export function LLMDecisionMatrix({ llmo }: LLMDecisionMatrixProps) {
  ...
  return (
    <section 
      id="decision-support"
      aria-label="Decision Support Framework and Architecture Trade-offs"
      className="py-24 px-4 sm:px-6 relative z-10 bg-[#050507]"
    >
      <div className="max-w-7xl mx-auto space-y-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="p-8 rounded-3xl bg-white/[0.015] border border-emerald-500/20 relative overflow-hidden">
            ...
            <ul className="space-y-3.5">
              {llmo.whenToChoose.map((item, i) => (
                <li key={i} className="flex items-start gap-3 text-xs md:text-sm text-white/75 font-sans leading-relaxed">
          ...
          <div className="p-8 rounded-3xl bg-white/[0.015] border border-rose-500/20 relative overflow-hidden">
            ...
            <ul className="space-y-3.5">
              {llmo.whenNotToChoose.map((item, i) => (
                <li key={i} className="flex items-start gap-3 text-xs md:text-sm text-white/75 font-sans leading-relaxed">
        ...
        {llmo.tradeoffs && llmo.tradeoffs.length > 0 && (
        ...
        {llmo.commonMistakes && llmo.commonMistakes.length > 0 && (
          <div className="space-y-6">
            <div className="flex items-center gap-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {llmo.commonMistakes.map((m, i) => (
                <div key={i} className="p-6 rounded-2xl bg-white/[0.01] border border-amber-500/15 flex flex-col justify-between space-y-4">
                  <div>
