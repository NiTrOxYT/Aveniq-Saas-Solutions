...
import { Cpu, Layers, Wrench, ChevronRight, HelpCircle } from "lucide-react";
import type { LLMOData } from "@/data/serviceData";

interface LLMArchitectureSectionProps {
  llmo: LLMOData;
}

export function LLMArchitectureSection({ llmo }: LLMArchitectureSectionProps) {
  ...
  return (
    <section 
      ...
      aria-label="System Architecture and Technical Implementation"
      className="py-24 px-4 sm:px-6 relative z-10 bg-black"
    >
      <div className="max-w-7xl mx-auto space-y-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            <div className="space-y-4">
              {llmo.implementationSteps.map((step, idx) => (
                <div key={idx} className="p-4 rounded-2xl bg-white/[0.015] border border-white/[0.06] flex items-start gap-4">
        ...
        {llmo.entities && llmo.entities.length > 0 && (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-8">
                {llmo.entities[activeEntityIndex] && (
                  <div className="p-8 rounded-3xl bg-[#09090d] border border-white/[0.08] space-y-6">
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-white/[0.06]">
                        <div>
                          ...
                          <ul className="text-white/60 font-mono text-xs space-y-1">
                            {llmo.entities[activeEntityIndex].dependencies.map((d, i) => (
                              <li key={i}>• {d}</li>
                            ))}
                          </ul>
                        </div>
                        ...
                        <div>
                          ...
                          <ul className="text-white/60 font-mono text-xs space-y-1">
                            {llmo.entities[activeEntityIndex].alternatives.map((a, i) => (
                              <li key={i}>• {a}</li>
                            ))}
                          </ul>
                        </div>
                        ...
                        <div>
                          ...
                          <ul className="text-white/60 font-mono text-xs space-y-1">
                            {llmo.entities[activeEntityIndex].limitations.map((l, i) => (
                              <li key={i}>• {l}</li>
                            ))}
                          </ul>
                        </div>
        ...
        {llmo.troubleshooting && llmo.troubleshooting.length > 0 && (
