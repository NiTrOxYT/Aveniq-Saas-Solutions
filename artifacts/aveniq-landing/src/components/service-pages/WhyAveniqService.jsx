...
export function WhyAveniqService() {
    ...
    return (<section className="py-28 px-4 sm:px-6 relative z-10 border-t border-white/[0.04]">
      ...
      <div className="absolute inset-0 pointer-events-none" style={{
            background: "radial-gradient(ellipse 50% 60% at 10% 50%, rgba(103,80,164,0.05) 0%, transparent 70%)",
        }} aria-hidden="true"/>
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <motion.div initial={reduce ? false : { opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.7, ease: [0.23, 1, 0.32, 1] }} className="mb-16 max-w-2xl">
          <h2 className="text-3xl md:text-5xl font-medium text-white mb-5 tracking-tight leading-tight" style={{ fontFamily: "'Instrument Serif', serif" }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-white/[0.04] rounded-2xl overflow-hidden border border-white/[0.04]">
          {WHY_ITEMS.map((item, i) => {
            ...
            return (<motion.div key={i} initial={reduce ? false : { opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{
                    duration: 0.5,
                    delay: i * 0.06,
                    ease: [0.23, 1, 0.32, 1],
                ...
                {/* Hover corner glow */}
                <div className="absolute top-0 left-0 w-full h-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" style={{
                    background: "radial-gradient(circle at top left, rgba(103,80,164,0.08), transparent 60%)",
                }}/>
                ...
                <div className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0" style={{
                  ...
                  <Icon className="w-4 h-4 text-[#9C89D9]" strokeWidth={1.5}/>
                </div>
