...
export function SEOHead({ service }: SEOHeadProps) {
  useEffect(() => {
    ...
    if (techArticleSchema) injectScript("schema-techarticle", JSON.stringify(techArticleSchema));
    if (howToSchema) injectScript("schema-howto", JSON.stringify(howToSchema));
    if (definedTermSetSchema) injectScript("schema-definedterms", JSON.stringify(definedTermSetSchema));
