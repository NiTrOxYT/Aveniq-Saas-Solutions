...
export function TechStack({ service }) {
    ...
    return (<section className="py-28 px-4 sm:px-6 relative z-10 border-t border-white/[0.04] bg-black">
      {/* Ambient right glow */}
      <div className="absolute inset-0 pointer-events-none" style={{
            background: "radial-gradient(ellipse 50% 50% at 90% 50%, rgba(103,80,164,0.04) 0%, transparent 70%)",
        }} aria-hidden="true"/>
          ...
          {categories.map((category, ci) => (<motion.div key={category} initial={reduce ? false : { opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{
              ...
              <div className="flex flex-wrap gap-2.5 pl-4">
                {grouped[category].map((tech, ti) => (<motion.div key={tech.name} initial={reduce ? false : { opacity: 0, scale: 0.94 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, amount: 0.2 }} transition={{
                    ...
                    <img src={`https://cdn.simpleicons.org/${tech.iconSlug}/ffffff`} alt={tech.name} width={14} height={14} className="w-3.5 h-3.5 opacity-40 group-hover/tech:opacity-90 group-hover/tech:scale-105 transition-all duration-300" loading="lazy" decoding="async" onError={(e) => {
