...
export default function HeroSection() {
    ...
    useEffect(() => {
        ...
        tl.fromTo(badgeRef.current, { y: 12, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6, ease: "power3.out" });
        tl.fromTo(line1Ref.current, { y: 80, opacity: 0, skewY: 3 }, { y: 0, opacity: 1, skewY: 0, duration: 1, ease: "power4.out" }, "-=0.2");
        tl.fromTo(line2Ref.current, { y: 80, opacity: 0, skewY: 3 }, { y: 0, opacity: 1, skewY: 0, duration: 1, ease: "power4.out" }, "-=0.75");
        tl.fromTo(glowRef.current, { scaleX: 0, opacity: 0 }, { scaleX: 1, opacity: 1, duration: 0.9, ease: "power3.inOut" }, "-=0.5");
        tl.fromTo(descRef.current, { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.7, ease: "power3.out" }, "-=0.4");
    ...
    return (<section className="relative min-h-[100dvh] max-h-[1100px] flex flex-col items-center pt-20 sm:pt-24 pb-6 px-4 sm:px-6 z-10">
      ...
      {/* Headline */}
      <h1 className="text-center mb-6 md:mb-8 max-w-[1000px] mx-auto" style={{ fontFamily: "'Instrument Serif', serif", lineHeight: 1.15, letterSpacing: "-0.035em" }}>
      ...
      {/* Description */}
      <p ref={descRef} className="text-center max-w-lg mx-auto mb-8 md:mb-10 leading-[1.75] font-light px-2" style={{ color: "rgba(255,255,255,0.45)", fontSize: "1rem", fontFamily: "Barlow, sans-serif", letterSpacing: "0.01em" }}>
      ...
      <div ref={ctaRef} className="flex flex-col sm:flex-row items-center gap-3 mb-12 md:mb-16">
        ...
        <button ref={primaryBtnRef} data-testid="button-start-project" onClick={handleStartProjectClick} className="group relative px-9 py-3.5 rounded-full text-sm font-semibold text-white overflow-hidden active:scale-[0.97] transition-all duration-200 cursor-pointer" style={{ willChange: "transform" }}>
        ...
        {/* Secondary */}
        <button data-testid="button-view-work" onClick={handleViewWorkClick} className="group px-8 py-3.5 rounded-full text-sm font-medium flex items-center gap-2 active:scale-[0.97] transition-all duration-200 cursor-pointer" style={{ color: "rgba(255,255,255,0.5)", border: "1px solid rgba(255,255,255,0.08)" }} onMouseEnter={e => { e.currentTarget.style.color = "rgba(255,255,255,0.85)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.18)"; }} onMouseLeave={e => { e.currentTarget.style.color = "rgba(255,255,255,0.5)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.08)"; }}>
          View Our Work
