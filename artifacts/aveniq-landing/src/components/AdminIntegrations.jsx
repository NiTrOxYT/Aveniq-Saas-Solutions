...
import { Mail, RefreshCw, Layers, Globe, Activity, AlertCircle, CheckCircle2, ArrowRight } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
export const AdminIntegrations = ({ session }) => {
    ...
    const getHealthBadge = (health) => {
        switch (health) {
            case "healthy":
                return (<span className="flex items-center gap-1.5 px-2 py-1 rounded bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] font-mono">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"/> Healthy
          </span>);
            case "warning":
                return (<span className="flex items-center gap-1.5 px-2 py-1 rounded bg-amber-500/10 border border-amber-500/20 text-amber-400 text-[10px] font-mono">
            <span className="w-1.5 h-1.5 rounded-full bg-amber-400"/> Action Required
          </span>);
            case "critical":
                return (<span className="flex items-center gap-1.5 px-2 py-1 rounded bg-rose-500/10 border border-rose-500/20 text-rose-400 text-[10px] font-mono">
            <span className="w-1.5 h-1.5 rounded-full bg-rose-400"/> Error
          </span>);
            default:
                return (<span className="flex items-center gap-1.5 px-2 py-1 rounded bg-zinc-500/10 border border-zinc-500/20 text-zinc-400 text-[10px] font-mono">
    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-[#0e0e11] border border-[#1a1a22] rounded-xl p-6 sm:p-8 space-y-6">
        <div className="border-b border-[#1a1a22] pb-4 flex items-center justify-between">
          ...
          <button onClick={fetchStatus} disabled={loading} className="p-1 text-[#a1a1aa] hover:text-white hover:bg-white/[0.04] rounded transition-colors disabled:opacity-50" title="Refresh Status">
            <RefreshCw className={`w-3.5 h-3.5 ${loading ? "animate-spin text-[#10b981]" : ""}`}/>
          </button>
        </div>

        {loading ? (<div className="py-8 flex flex-col items-center justify-center space-y-3">
            ...
            <div className="lg:col-span-2 space-y-5">
              ...
              {status?.ipError && (<div className="p-4 bg-rose-500/10 border border-rose-500/20 text-rose-400 rounded-lg flex items-start gap-2.5 text-xs font-light leading-relaxed">
              ...
              {status?.success === false && (<div className="p-4 bg-rose-500/10 border border-rose-500/20 text-rose-400 rounded-lg flex items-start gap-2.5 text-xs font-light leading-relaxed">
                  <AlertCircle className="w-4 h-4 text-rose-400 shrink-0 mt-0.5"/>
                  <div className="space-y-2 w-full">
                    ...
                    <p className="text-[11px]">
                      ...
                      {status.isTransactionalKey === false && (<span className="block mt-1 text-amber-400 font-normal">
                    ...
                    {status.brevoError && (<pre className="p-2 bg-black/40 border border-white/5 rounded font-mono text-[10px] text-[#a1a1aa] overflow-x-auto whitespace-pre-wrap max-h-40">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-4 bg-[#08080a] border border-[#1a1a22] rounded-lg space-y-1.5">
                  ...
                  <p className="text-xs font-semibold text-white">
                    {status?.apiKeyPresent ? (<span className="text-emerald-400 flex items-center gap-1.5">
                        <CheckCircle2 className="w-3.5 h-3.5"/> Present
                      </span>) : (<span className="text-rose-400 flex items-center gap-1.5">
                ...
                <div className="p-4 bg-[#08080a] border border-[#1a1a22] rounded-lg space-y-1.5">
                  ...
                  <p className="text-xs font-semibold text-white">
                    {status?.apiReachable ? (<span className="text-emerald-400 flex items-center gap-1.5">
                ...
                <div className="p-4 bg-[#08080a] border border-[#1a1a22] rounded-lg space-y-1.5">
                  ...
                  <p className="text-xs font-semibold text-white">
                    {status?.senderVerified ? (<span className="text-emerald-400 flex items-center gap-1.5">
                ...
                <div className="p-4 bg-[#08080a] border border-[#1a1a22] rounded-lg space-y-1.5">
                  ...
                  <p className="text-xs font-semibold text-white">
                    {status?.domainVerified ? (<span className="text-emerald-400 flex items-center gap-1.5">
                ...
                <div className="sm:col-span-2 p-4 bg-[#08080a] border border-[#1a1a22] rounded-lg space-y-1.5">
                  ...
                  <p className="text-xs font-semibold text-white">
                    {status?.lastEmailSent ? (<span className="text-white text-[11px] font-mono">
                        {new Date(status.lastEmailSent).toLocaleString()}
                      </span>) : (<span className="text-[#a1a1aa]/40 italic">No data available yet</span>)}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 bg-[#08080a] border border-[#1a1a22] rounded-xl flex flex-col justify-between">
              ...
              <form onSubmit={handleSendTestEmail} className="space-y-4 text-xs">
                <div>
                  ...
                  <input type="email" id="test-email-input" value={testEmail} onChange={(e) => setTestEmail(e.target.value)} placeholder="admin@theaveniq.site" className="w-full bg-[#0e0e11] border border-[#1a1a22] rounded-lg px-3 py-2 focus:outline-none focus:border-[#10b981] text-white font-light text-xs" disabled={sendingTest || !status?.configured} required/>
                </div>

                <button type="submit" disabled={sendingTest || !status?.configured} className="w-full bg-[#F5F5F5] hover:bg-white text-black py-2.5 rounded-lg font-bold text-[11px] transition-colors flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
                  {sendingTest ? (<div className="w-3.5 h-3.5 rounded-full border border-black/20 border-t-black animate-spin"/>) : (<>Send Test Email <ArrowRight className="w-3 h-3"/></>)}
                </button>
              </form>
            </div>
          </div>)}
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-[#0e0e11] border border-[#1a1a22] rounded-xl p-6 flex flex-col justify-between opacity-60">
            ...
            <div className="pt-5">
              <button disabled className="px-4 py-2 bg-white/5 border border-white/10 text-white/30 rounded-lg text-[10px] font-semibold cursor-not-allowed">
                Configure
              </button>
            </div>
          </div>
