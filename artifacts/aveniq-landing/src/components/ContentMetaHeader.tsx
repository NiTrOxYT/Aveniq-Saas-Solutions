...
export interface ContentMetaHeaderProps {
  title: string;
  subtitle?: string;
  author?: string;
  reviewer?: string;
...
export default function ContentMetaHeader({
  title,
  subtitle,
  author = "Aveniq Engineering Team",
  ...
  return (
    <div className="mb-12 border-b border-white/10 pb-8">
      {category && (
        <span className="inline-block text-[#9C89D9] text-xs font-semibold uppercase tracking-widest mb-3 bg-[#6750A4]/15 px-3 py-1 rounded-full border border-[#9C89D9]/20">
          {category}
        </span>
      )}
      ...
      {subtitle && (
        <p className="text-white/60 text-lg md:text-xl max-w-3xl font-light leading-relaxed mb-6">
          {subtitle}
        </p>
      )}
