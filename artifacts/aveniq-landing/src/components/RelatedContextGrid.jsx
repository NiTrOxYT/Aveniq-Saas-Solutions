...
export default function RelatedContextGrid({ items, heading = "Explore Related Engineering Intelligence", subheading = "Deepen your knowledge across interconnected services, technologies, solutions, and architectural blueprints.", }) {
    if (!items || items.length === 0)
        return null;
    return (<section className="py-16 border-t border-white/10 my-16">
      <div className="mb-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {items.map((item, idx) => {
            ...
            return (<Link key={idx} href={item.href} className="group p-5 rounded-xl bg-zinc-950/60 border border-white/10 hover:border-[#9C89D9]/50 hover:bg-zinc-900/60 transition-all duration-300 flex flex-col justify-between">
              <div>
