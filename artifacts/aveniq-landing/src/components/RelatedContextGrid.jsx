...
export default function RelatedContextGrid({ items, heading = "Explore Related Engineering Intelligence", subheading = "Deepen your knowledge across interconnected services, technologies, solutions, and architectural blueprints.", }) {
    if (!items || items.length === 0)
        return null;
    return (<section className="py-28 px-4 sm:px-6 relative z-10 border-t border-white/[0.04]">
      <div className="max-w-7xl mx-auto">
        <div className="mb-12 max-w-2xl">
          <h3 className="text-3xl md:text-4xl font-medium text-white mb-4 tracking-tight" style={{ fontFamily: "'Instrument Serif', serif" }}>
            {heading}
          </h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item, idx) => {
            ...
            return (<Link key={idx} href={item.href} onMouseEnter={() => prefetchRoute(item.href)} onFocus={() => prefetchRoute(item.href)} className="group p-6 rounded-2xl bg-white/[0.03] backdrop-blur-[20px] border border-white/[0.08] hover:border-white/[0.18] shadow-[0_30px_80px_rgba(0,0,0,0.35)] hover:-translate-y-1.5 transition-all duration-500 flex flex-col justify-between overflow-hidden">
                <div>
