...
function CountUpNumber({ value, suffix, isFloat }) {
    ...
    return (<span ref={elementRef} className="tabular-nums">
      {current}
      {suffix}
    </span>);
}
export default function StatsBanner() {
    const reduce = useReducedMotion();
    return (<section className="relative py-12 md:py-16 border-y border-white/5 bg-black/40 backdrop-blur-md z-10 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 sm:gap-12 md:gap-16">
          {stats.map((s, i) => (<motion.div key={i} initial={reduce ? { opacity: 1 } : { opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.6, delay: i * 0.1, ease: [0.23, 1, 0.32, 1] }} className="flex flex-col items-center text-center gap-2">
              <span className="text-4xl md:text-5xl font-bold font-serif" style={{
